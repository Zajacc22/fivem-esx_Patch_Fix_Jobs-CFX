...
local hashSkin = GetHashKey("mp_m_freemode_01") -- Votre Skin Online pour permette de recevoir la tenue sans envoyée de requete ! ^^
RegisterNUICallback('select', function(data, cb)
	...
	if data.menu == 'cloakroom' then
		...
		if data.val == 'lumberjack_wear' then
			if(GetEntityModel(GetPlayerPed(-1)) == hashSkin) then -- Permet de recupérer votre Skin
			   SetPedComponentVariation(GetPlayerPed(-1), 11, 41, 0, 2)  -- Voici la Tenue Bucheron
			   ...
			   SetPedComponentVariation(GetPlayerPed(-1), 3, 6, 0, 2)   -- under sk
			end
		end
